...
module CamlBuilder {
  ...
  class ViewInternal implements IView {
    ...
    View(viewFields?: ViewField[]): IView {
      ...
      if (viewFields) {
        ...
        for (let viewField of viewFields) {
          if (typeof viewField === "string") fieldNames.push(viewField);
          else aggregations.push(viewField);
        }
        if (fieldNames.length > 0) this.CreateViewFields(fieldNames);
        if (aggregations.length > 0) this.CreateAggregations(aggregations);
      }
    ...
    Scope(scope: ViewScope): IView {
      switch (scope) {
        ...
        case ViewScope.RecursiveAll:
          this.builder.SetAttributeToLastElement(
            "View",
            "Scope",
            "RecursiveAll"
          );
        ...
        default:
          throw new Error(
            "Incorrect view scope! Please use values from CamlBuilder.ViewScope enumeration."
          );
      }
      return this;
    }
    InnerJoin(
      ...
      return this.joinsManager.Join(
        lookupFieldInternalName,
        alias,
        "INNER",
        fromList
      );
    ...
    Select(
      ...
      return this.joinsManager.ProjectedField(
        remoteFieldInternalName,
        remoteFieldAlias
      );
    }
    ToString(): string {
    ...
    Finalize() {
      if (this.joins.length > 0) {
        ...
        for (var i = 0; i < this.joins.length; i++) {
          ...
          if (join.FromList) fieldAttrs["List"] = join.FromList;
  ...
  class QueryToken implements IExpression {
    ...
    And(): IFieldExpression {
      ...
      const amountOfUnclosedBefore = this.builder.tree
        .slice(0, this.startIndex)
        .reduce(
          (count, e) => (count += e.Kind === "Start" && e.End === null ? 1 : 0),
          0
        );
  ...
  class RawQueryInternal implements IRawQuery, IRawQueryModify {
    ...
    public OverrideQueryParams(
      ...
      if (rowLimit) {
        ...
        const attributes = paged ? [{ Name: "Paged", Value: "TRUE" }] : [];
        viewbuilder.WriteStart("RowLimit", attributes);
        // We set the <RowLimit> inner value to the actual row limit provided
        viewbuilder.tree.push({ Kind: "Raw", Xml: "" + rowLimit });
      ...
      var whereBuilder = this.parseRecursive(
        builder,
    ...
    private parseRecursive(
      ...
      var attrs = [];
      for (
        var i = 0, len = (<HTMLElement>node).attributes.length;
        i < len;
        i++
      ) {
        attrs.push({
          Name: (<HTMLElement>node).attributes[i].name,
          Value: (<HTMLElement>node).attributes[i].value,
        });
      }
      ...
      for (var i = 0, len = node.childNodes.length; i < len; i++) {
        ...
        var result = this.parseRecursive(
          builder,
          node.childNodes[i],
          modifyType
        );
  ...
  class FieldExpression implements IFieldExpression {
    ...
    /** Specifies that a condition will be tested against the field with the specified internal name, and the type of this field is LookupMulti */
    LookupMultiField(internalName: string): ILookupMultiFieldExpression {
      return new LookupOrUserMultiFieldExpression(
        this.builder,
        internalName,
        FieldMultiExpressionType.LookupMulti
      );
    }
    /** Specifies that a condition will be tested against the field with the specified internal name, and the type of this field is UserMulti */
    UserMultiField(internalName: string): IUserMultiFieldExpression {
      return new LookupOrUserMultiFieldExpression(
        this.builder,
        internalName,
        FieldMultiExpressionType.UserMulti
      );
    }
    /** Specifies that a condition will be tested against the field with the specified internal name, and the type of this field is Date */
    DateField(internalName: string): IDateTimeFieldExpression {
  ...
  class LookupOrUserMultiFieldExpression
    implements ILookupMultiFieldExpression
  {
    constructor(
      builder: Builder,
      name: string,
      type: FieldMultiExpressionType
    ) {
      this.builder = builder;
      this.name = name;
      this.type = type;
      ...
      else this.typeAsString = "LookupMulti";
    }
    private builder: Builder;
    ...
    IncludesSuchItemThat(): any {
      ...
      else return new UserFieldExpression(this.builder, this.name);
    }
  ...
  class UserFieldExpression implements IUserFieldExpression {
    ...
    EqualToCurrentUser(): IExpression {
      ...
      this.builder.WriteBinaryOperation(
        this.startIndex,
        "Eq",
        "Integer",
        "{UserID}"
  ...
  class Builder {
    ...
    InsertAtIndexAndWriteEnd(
      index: number,
      tagName: string,
      attributes?: any[]
    ) {
      this.ThrowIfFinalized();
      const startElem: StartElement = {
        Kind: "Start",
        Name: tagName,
        Attributes: attributes,
        End: null,
    ...
    WriteGroupBy(groupFieldName, collapse, groupLimit) {
      ...
      if (this.unclosedTags.length > 0) {
        ...
        if (top.Name == "Query") tagsToClose--;
        else if (top.Name == "View") tagsToClose -= 2;
        this.WriteEnd(tagsToClose);
      }
      const attributes = [];
      if (collapse) attributes.push({ Name: "Collapse", Value: "TRUE" });
      if (groupLimit)
    ...
    WriteStartOrderBy(override, useIndexForOrderBy) {
      ...
      var attributes = [];
      if (override) attributes.push({ Name: "Override", Value: "TRUE" });
    ...
    WriteConditions(builders: Builder[], elementName: string) {
      ...
      for (var i = 0; i < builders.length; i++) {
        ...
        Array.prototype.splice.apply(
          this.tree,
          (<any[]>[pos, 0]).concat(conditionBuilder.tree)
        );
        if (i > 0) {
    ...
    ThrowIfFinalized() {
      if (this.finalString)
        throw new Error(
          "CamlBuilder was already finalized, you cannot make modifications to it anymore. Please create a new CamlBuilder object for every query."
        );
    }
    Finalize(): string {
      if (this.finalString) return this.finalString;

      if (this.unclosedTags.length > 0) this.WriteEnd(this.unclosedTags.length);
      ...
      for (var i = 0; i < this.tree.length; i++) {
        ...
        if (element.Kind == "FieldRef") {
          ...
          if (element.LookupId) xml += ' LookupId="TRUE"';
          if (element.Descending) xml += ' Ascending="FALSE"';
  ...
  export class CamlValues {
    ...
    static ListProperty = {
      ...
      MajorWithMinorVersionsLimit:
        '{ListProperty Name="MajorWithMinorVersionsLimit"}',
