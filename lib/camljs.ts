...
module CamlBuilder {
  ...
  class RawQueryInternal implements IRawQuery, IRawQueryModify {
    ...
    public OverrideQueryParams(
      ...
      if (rowLimit) {
        ...
        else viewbuilder.tree.push({ Element: "Start", Name: "RowLimit" });
  ...
  class Builder {
    ...
    SetAttributeToLastElement(
      ...
      throw new Error(
        "CamlJs ERROR: can't find element '" +
          tagName +
    ...
    WriteRowLimit(paged: boolean, limit: number) {
      ...
      else this.tree.push({ Element: "Start", Name: "RowLimit" });
    ...
    WriteStart(tagName: string, attributes?: any[]) {
      ...
      else this.tree.push({ Element: "Start", Name: tagName });
    }
    WriteEnd(count?: number) {
      this.ThrowIfSealed();
      if (count > 0) this.tree.push({ Element: "End", Count: count });
      else this.tree.push({ Element: "End" });
    }
    ...
    WriteStartGroupBy(groupFieldName, collapse, groupLimit) {
      ...
      if (this.unclosedTags > 0) {
        ...
        if (this.tree[0].Name == "Query") tagsToClose--;
        else if (this.tree[0].Name == "View") tagsToClose -= 2;
        if (tagsToClose > 0)
          this.tree.push({ Element: "End", Count: tagsToClose });
        this.unclosedTags -= tagsToClose;
      }
      let elem = { Element: "Start", Name: "GroupBy", Attributes: [] };
      if (collapse) elem.Attributes.push({ Name: "Collapse", Value: "TRUE" });
      if (groupLimit)
    ...
    WriteStartOrderBy(override, useIndexForOrderBy) {
      ...
      else this.tree.push({ Element: "Start", Name: "OrderBy" });
      this.unclosedTags++;
    }
    WriteConditions(builders: Builder[], elementName: string) {
      ...
      for (var i = 0; i < builders.length; i++) {
        ...
        Array.prototype.splice.apply(
          this.tree,
          [pos, 0].concat(conditionBuilder.tree)
        );
      }
    }
    ThrowIfSealed() {
      if (this.sealed)
        throw new Error(
          "CamlBuilder was already serialized, you cannot make modifications to it anymore. Please create a new CamlBuilder object for every query."
        );
    }
    Finalize(): string {
...
(function (window) {
  ...
  if (typeof window["SP"] == "undefined") {
    ...
    window["SP"].XmlWriter.prototype = {
      ...
      writeElementString: function SP_XmlWriter$writeElementString(
        tagName,
        value
      ) {
        this.$1R_0();
      ...
      writeAttributeString: function SP_XmlWriter$writeAttributeString(
        localName,
        value
      ) {
        if (this.$V_0) {
      ...
      writeStartAttribute: function SP_XmlWriter$writeStartAttribute(
        localName
      ) {
        if (!this.$V_0) {
          throw "Invalid operation";
